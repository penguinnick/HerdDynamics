...
#' @param male.offtake A vector of length=length of age classes giving the culling rates for males. If NULL (default) the culling rates in 'offtake' are used for both males and females.
...
build_param <- function(tcla, parms, phase, offtake, male.offtake = NULL, prolificacyRate = NULL, fbirthRate = 0.5, lambda = 1, correctionfec = TRUE, truncated = TRUE, method = "steady", hazards = FALSE) {
  ...
  with(parms, {
    ...
    if (truncated) {
      ...
      if (!is.null(male.offtake)) {
        z$hoff <- c(offtake, 1, male.offtake, 1)
      } else {
